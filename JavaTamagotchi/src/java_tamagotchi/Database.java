...
public class Database {
	...
	static boolean TableExistence() throws SQLException {
		...
		String query = "select count(*)as count from information_schema.tables  where table_name ='GAME'";
		...
		try {
			...
			infoBoxError("An error occurred while checking for the table existence, the program will shut down.",
					"Error");
			System.exit(0);
	...
	public static boolean gameExistence() throws SQLException {
		...
		boolean exist = false;
		int count = 0;
		String save = "select count(*)as count FROM GAME;";
	...
	static void load() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				...
				Pet.load(rs.getString("name"), rs.getString("sex"), rs.getInt("gold"), rs.getInt("health"),
						rs.getInt("energy"), rs.getInt("love"));

				Merchant.load(rs.getBoolean("item0"), rs.getBoolean("item1"), rs.getBoolean("item2"),
						rs.getBoolean("item3"));

				GUIinventory.load(rs.getString("inventory0"), rs.getString("inventory1"), rs.getString("inventory2"),
						rs.getString("inventory3"));

			}

		} catch (SQLException e) {

			infoBoxError("Si è verificato un errore durantela stampa della tabella, il programma verrà chiuso.",
					"Errore");
			System.exit(0);
